...
<div class="row">
    <div class="col-md-10">
        <?php if(isset($message)){ echo "<div class='alert alert-{$message['type']}' role='alert'>{$message['text']}</div>"; } ?>
        <h1><a href="<?php echo "/post/article/{$post->id}"?>"><?php echo $post->title; ?></a></h1>

        <p class="lead">
            <i class="fa fa-user"></i> pise
            <a href="">
                <?php echo $post->author; ?></a>
        </p>
        <hr>
        <p>
            <i class="fa fa-calendar"></i> Postavljeno
            <?php echo $post->datetime; ?>
        </p>
        <p>
            <i class="fa fa-tags"></i> Tagovi:
            <?php $ara=e xplode( ',', $post->tags); foreach($ara as $a){ echo "<a href=''><span class='badge badge-info'>{$a}</span></a> "; } ?>
...
<div class="row">
    ...
    <br>
    <div class="col-md-10">
        <?php if(!isset($comments) || $comments==F ALSE) { echo "<div class='alert alert-info' role='alert'>Nema komentara, komentarisite prvi</div>"; } else { ?>
        <section class="comment-list">
            <!-- First Comment -->
            <?php foreach ($comments as $comment){ $x=i sset($comment->depth) ? $comment->depth : 0; ?>
                ...
                <div class="col-md-2 col-sm-2 <?php
                    ...
                    <figure class="thumbnail">
                        ...
                        <figcaption class="text-center">
                            <?php echo $comment->comm_author_name ;?></figcaption>
                    </figure>
                </div>
                <!-- col-md-9 col-sm-9 -->
                        ...
                        <div class="panel-body">
                            <header class="text-left">
                                <div class="comment-user">
                                    ...
                                    <p style="float:right;">
                                        <button class="btn btn-danger report_b" data="<?php echo $comment->comment_id;?>" data-container="body" data-toggle="popover" data-placement="top" data-content="Hvala, vas zahtjev bice razmotren." data-original-title="" title="" <?php if (in_array($comment->comment_id, $disabled_report)) { echo "disabled"; }?> >
                                            <i class="fa fa-warning"></i> Prijavi Komentar
                                        </button>
                                    </p>
                                </div>
                                <time class="comment-date" datetime="16-12-2014 01:05"><i class="fa fa-clock-o"></i>
                                    <?php echo $comment->dtime ;?></time>
                            </header>
        ...
        <?php } // foreach commnets as comment } // else block closed ?>
    </div>
</div>
